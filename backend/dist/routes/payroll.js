  ...
  `;
    ...
    records.forEach(record => {
        ...
        if (record.clock_in_time && record.clock_out_time) {
            const clockIn = (0, date_fns_1.parseISO)(record.clock_in_time);
            const clockOut = (0, date_fns_1.parseISO)(record.clock_out_time);
            actualHours = (0, date_fns_1.differenceInHours)(clockOut, clockIn, { roundingMethod: 'round' });
        }
        else if (record.shift_start && record.shift_end) {
            // 使用排班时间估算
            const shiftStart = (0, date_fns_1.parseISO)(`${record.date}T${record.shift_start}`);
            const shiftEnd = (0, date_fns_1.parseISO)(`${record.date}T${record.shift_end}`);
            actualHours = (0, date_fns_1.differenceInHours)(shiftEnd, shiftStart);
            isMissingClock = true;
        }
        if (actualHours > 0) {
