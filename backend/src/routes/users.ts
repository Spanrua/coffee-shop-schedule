...
import { buildStoreFilter, getManageableStoreContext, normalizeAdminScope } from '../utils/storeAccess';

const router = Router();

function normalizeStoreIds(value: unknown): number[] {
...
function replaceStoreAccess(userId: number, accessType: 'support' | 'manage', storeIds: number[]) {
  db.prepare('DELETE FROM user_store_access WHERE user_id = ? AND access_type = ?').run(userId, accessType);
...
function hydrateUsers(rows: Array<Omit<User, 'password_hash'>>) {
  const supportRows = db.prepare(`
    SELECT user_id, store_id FROM user_store_access WHERE access_type = 'support'
  `).all() as Array<{ user_id: number; store_id: number }>;
  const manageRows = db.prepare(`
...
router.get('/', authenticate, requireAdmin, (req: AuthRequest, res) => {
  try {
    ...
    if (!context.isSuperAdmin) {
      const filter = buildStoreFilter('usa', context.storeIds);
      query += filter.clause;
    ...
    const users = db.prepare(query).all(...params) as Array<Omit<User, 'password_hash'>>;
...
router.post('/', authenticate, requireAdmin, async (req: AuthRequest, res) => {
  ...
  const normalizedAdminScope = normalizeAdminScope(role, admin_scope);
  ...
  try {
    ...
    const result = db.prepare(
      'INSERT INTO users (username, password_hash, name, role, admin_scope, primary_store_id, hourly_rate) VALUES (?, ?, ?, ?, ?, ?, ?)'
    ).run(username, passwordHash, name, role, normalizedAdminScope, primaryStoreId, hourly_rate || 50.0);
...
router.put('/:id', authenticate, requireAdmin, async (req: AuthRequest, res) => {
  ...
  try {
    const user = db.prepare('SELECT id, role, primary_store_id FROM users WHERE id = ?').get(userId) as User | undefined;
    ...
    if (admin_scope !== undefined) {
      const normalized = normalizeAdminScope(role || user.role, admin_scope);
      query += ', admin_scope = ?';
    ...
    const updated = db.prepare('SELECT id, role, admin_scope, primary_store_id FROM users WHERE id = ?')
      .get(userId) as User;
    const supportStores = normalizeStoreIds(support_store_ids);
    const managedStores = normalizeStoreIds(managed_store_ids);
    ...
    if (managed_store_ids !== undefined || admin_scope !== undefined || role !== undefined) {
      const scope = normalizeAdminScope(updated.role, updated.admin_scope);
      if (scope === 'super') {
